"""Train and evaluate the single-scale 13x13 CNN-DRE model."""
...
DEFAULT_OUTPUT_DIR = Path("outputs/scale_based_models/scale_13")
...
# =========================================================
# 13x13 ENCODER
# Exact counterpart of the 13x13 branch in the multi-scale model.
# =========================================================
class PatchEncoder13HybridDil2(nn.Module):
    """
    13x13 encoder:
      - channel-aware partial conv at input
      - spatial partial conv thereafter
      - one downsample 13->6
      - one dilated (d=2) block after pooling to expand context without more pooling
      - masked GAP
    """
    def __init__(self, in_value_channels: int, emb_dim: int = 32):
        ...
        self.b4 = CPConvBlock(32, 32, dilation=2)   # <-- NEW dilated block
    ...
    def forward(self, x_val, x_mask):
        ...
        x, m = self.b1(x_val, m_cov)  # (B,16,13,13), m (B,1,13,13)
        x, m = self.b2(x, m)

        x, m = self.pool(x, m)        # (B,16,6,6)

        x, m = self.b3(x, m)          # (B,32,6,6)
        x, m = self.b4(x, m)          # (B,32,6,6) with dilation=2
        x, m = self.b5(x, m)          # (B,32,6,6)
...
class CNNDRE(nn.Module):
    ...
    def __init__(self, in_value_channels, emb_dim=32, hidden_dims=(32,), dropout=0.35, clip_logits=10.0):
        super().__init__()
        self.encoder = PatchEncoder13HybridDil2(in_value_channels, emb_dim)
    ...
    for ep in range(1, MAX_EPOCHS + 1):
        ...
        if auc_ok:
            state = {
                ...
                "model_type": "CNNDRE",
                "encoder_type": "PatchEncoder13HybridDil2",
                "in_value_channels": in_value_channels,
...
def ensemble_predict_on_indices_cnn(
    ...
    for fid in fold_ids:
        ...
        if state.get("encoder_type") != "PatchEncoder13HybridDil2":
            raise RuntimeError(
                f"Encoder mismatch for fold {fid}: expected PatchEncoder13HybridDil2, "
...
# =========================================================
def parse_args():
    parser = argparse.ArgumentParser(description="Train the single-scale 13x13 CNN-DRE model.")
    parser.add_argument(
