...
"""Train and evaluate the multiscale (3, 13, 33) CNN-DRE model."""
...
# =========================================================
# IMPORTANT:
# This multi-scale model expects the STORED input patches to be the
# LARGEST scale (default: 33x33). The 13x13 and 3x3 patches are
# extracted as centered crops inside the model.
#
# If your current .npy tensors are still 13x13, you must regenerate the
# patch dataset at 33x33 first. You cannot infer 33x33 context from a
# stored 13x13 patch.

DEFAULT_OUTPUT_DIR = Path("outputs/scale_based_models/multiscale_cnn_dre")
...
# Largest stored patch size
PATCH_SIZE = 33

# Multi-scale centered crops extracted from the largest stored patch
MULTISCALE_SIZES = (3, 13, 33)
...
# Ranking loss hyperparams
LAMBDA_RANK = 0.05
MAX_RANK_PAIRS = 4096
RANK_TEMPERATURE = 1.0

# Softmax(AUC) weighting hyperparam
...
def center_crop_4d(x: torch.Tensor, crop_size: int) -> torch.Tensor:
    """
    Center crop for tensors with shape (B, C, H, W).
    """
    if x.dim() != 4:
        raise ValueError(f"Expected 4D tensor, got {tuple(x.shape)}")
    _, _, h, w = x.shape
    if crop_size > h or crop_size > w:
        raise ValueError(f"Requested crop {crop_size}, but input is {(h, w)}")
...
# =========================================================
# Ranking component for classifier-based DRE
# =========================================================
...
# =========================================================
# Single-scale branch encoders used inside the multi-scale model
# =========================================================
class PatchEncoder3Hybrid(nn.Module):
    """
    Branch for 3x3 centered crop.
...
class PatchEncoder13HybridDil2(nn.Module):
    """
    Branch for 13x13 centered crop.
    Same logic as the original single-scale encoder.
    """
    def __init__(self, in_value_channels: int, emb_dim: int = 32):
        super().__init__()
        self.b1 = CPConvBlock1(in_value_channels, 16)
        ...
        self.b3 = CPConvBlock(16, 32, dilation=1)
        self.b4 = CPConvBlock(32, 32, dilation=2)
...
class PatchEncoder33HybridDil2(nn.Module):
    """
    Branch for 33x33 full patch.
    Two resolutions: capture broader landscape context before GAP.
    """
    def __init__(self, in_value_channels: int, emb_dim: int = 32):
        super().__init__()
        self.b1 = CPConvBlock1(in_value_channels, 16)
        ...
        self.pool1 = MaskedMaxPool2d(2, 2)  # 33 -> 16

        self.b3 = CPConvBlock(16, 32, dilation=1)
        self.b4 = CPConvBlock(32, 32, dilation=2)

        self.pool2 = MaskedMaxPool2d(2, 2)  # 16 -> 8

        self.b5 = CPConvBlock(32, 32, dilation=1)
        self.b6 = CPConvBlock(32, 32, dilation=2)
...
class MultiScaleFusion(nn.Module):
    """
    Learns scale weights alpha_3, alpha_13, alpha_33 from the concatenated
    scale-specific embeddings. The fused representation is a weighted
    concatenation so each branch remains explicitly recoverable.
    """
    def __init__(self, emb_dim: int, n_scales: int, hidden_dim: int = 32):
        super().__init__()
        self.n_scales = n_scales
        self.emb_dim = emb_dim
        self.gate = nn.Sequential(
            nn.Linear(emb_dim * n_scales, hidden_dim),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_dim, n_scales),
        )

    def forward(self, z_list: Sequence[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        if len(z_list) != self.n_scales:
            raise ValueError(f"Expected {self.n_scales} scale embeddings, got {len(z_list)}")
        z_cat = torch.cat(z_list, dim=1)
        alpha = torch.softmax(self.gate(z_cat), dim=1)  # (B, n_scales)

        weighted_parts = []
        for i, z in enumerate(z_list):
            weighted_parts.append(z * alpha[:, i:i + 1])
        z_fused = torch.cat(weighted_parts, dim=1)
        return z_fused, alpha


class MultiScaleCNNDRE(nn.Module):
    """
    Multi-scale classifier-based DRE:
      largest stored patch -> centered crops (3, 13, 33)
      -> three mask-aware encoders -> learned scale fusion -> DRE head
    """
    def __init__(
        self,
        ...
        scales: Sequence[int] = (3, 13, 33),
        max_patch_size: int = 33,
        ...
        if sorted(self.scales) != [3, 13, 33]:
            raise ValueError("This implementation expects scales=(3,13,33)")
        if self.max_patch_size < max(self.scales):
            raise ValueError("max_patch_size must be >= largest scale")

        self.encoder_3 = PatchEncoder3Hybrid(in_value_channels, emb_dim)
        self.encoder_13 = PatchEncoder13HybridDil2(in_value_channels, emb_dim)
        self.encoder_33 = PatchEncoder33HybridDil2(in_value_channels, emb_dim)
        ...
        self.dre_head = MLPDRE(
            in_dim=emb_dim * len(self.scales),
            hidden_dims=hidden_dims,
            dropout=dropout,
            clip_logits=clip_logits,
        )

    def forward(self, x_val, x_mask, return_aux: bool = False):
        if x_val.dim() != 4 or x_mask.dim() != 4:
            raise ValueError(f"Expected x_val,x_mask 4D. Got {x_val.shape}, {x_mask.shape}")
        if x_val.size(-1) < self.max_patch_size or x_val.size(-2) < self.max_patch_size:
            raise ValueError(
                f"Input patch is too small for multi-scale model. "
                f"Need at least {self.max_patch_size}x{self.max_patch_size}, got {tuple(x_val.shape[-2:])}"
            )

        x3 = center_crop_4d(x_val, 3)
        m3 = center_crop_4d(x_mask, 3)

        x13 = center_crop_4d(x_val, 13)
        m13 = center_crop_4d(x_mask, 13)

        x33 = center_crop_4d(x_val, 33)
        m33 = center_crop_4d(x_mask, 33)

        z3 = self.encoder_3(x3, m3)
        z13 = self.encoder_13(x13, m13)
        z33 = self.encoder_33(x33, m33)

        z_fused, alpha = self.fusion([z3, z13, z33])
        logits = self.dre_head(z_fused)
        ...
        if return_aux:
            aux = {
                ...
                "z13": z13,
                "z33": z33,
                "alpha": alpha,     # columns correspond to scales (3,13,33)
                "z_fused": z_fused,
            }
            return logits, aux
        return logits
...
def train_and_save_fold_model_cnn(
    ...
    model = MultiScaleCNNDRE(
        in_value_channels=in_value_channels,
        emb_dim=EMB_DIM,
        hidden_dims=HIDDEN_DIMS,
        dropout=DROPOUT,
        clip_logits=CLIP_LOGITS,
        scales=MULTISCALE_SIZES,
    ...
    for ep in range(1, MAX_EPOCHS + 1):
        ...
        if auc_ok:
            state = {
                ...
                "model_type": "MultiScaleCNNDRE",
                "in_value_channels": in_value_channels,
                "patch_size": PATCH_SIZE,
                "multiscale_sizes": tuple(MULTISCALE_SIZES),
                "emb_dim": EMB_DIM,
                "hidden_dims": HIDDEN_DIMS,
                "fusion_hidden_dim": FUSION_HIDDEN_DIM,
                "pi0": float(pi0_fold),
...
def ensemble_predict_on_indices_cnn(
    ...
    for fid in fold_ids:
        ...
        if tuple(state.get("multiscale_sizes", tuple(MULTISCALE_SIZES))) != tuple(MULTISCALE_SIZES):
            raise RuntimeError(f"Scale mismatch for fold {fid}")
        ...
        model = MultiScaleCNNDRE(
            ...
            dropout=DROPOUT,
            clip_logits=CLIP_LOGITS,
            scales=tuple(state.get("multiscale_sizes", MULTISCALE_SIZES)),
            max_patch_size=state.get("patch_size", PATCH_SIZE),
            fusion_hidden_dim=state.get("fusion_hidden_dim", FUSION_HIDDEN_DIM),
        ).to(device)
...
# =========================================================
# Optional utility: extract learned scale weights alpha for interpretation
# =========================================================
def predict_scale_weights_on_indices(
    X_values: np.ndarray,
    X_masks: np.ndarray,
    indices: np.ndarray,
    model_path: str,
    ...
    """
    Returns mean learned scale weights alpha for each sample using one trained model.
    Output shape: (N, 3), columns correspond to scales (3,13,33).
    """
    state = torch.load(model_path, map_location="cpu")
    model = MultiScaleCNNDRE(
        in_value_channels=state["in_value_channels"],
        emb_dim=state.get("emb_dim", EMB_DIM),
        hidden_dims=tuple(state.get("hidden_dims", HIDDEN_DIMS)),
        dropout=DROPOUT,
        clip_logits=CLIP_LOGITS,
    ...
    with torch.no_grad():
        for start in range(0, len(idx), BATCH_SIZE):
            ...
            _, aux = model(xv, xm, return_aux=True)
            alpha = aux["alpha"].cpu().numpy()
            all_alpha.append(alpha)

    return np.concatenate(all_alpha, axis=0)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train the multiscale CNN-DRE model using 33x33 stored patches."
    )
    parser.add_argument(
        "--cv-dir",
        type=Path,
...
if __name__ == "__main__":
    ...
    if X_cv.shape[-1] != PATCH_SIZE or X_cv.shape[-2] != PATCH_SIZE:
        raise ValueError(
            f"Expected stored CV patches of size {PATCH_SIZE}x{PATCH_SIZE}, got {tuple(X_cv.shape[-2:])}"
        )
    ...
    if X_test.shape[-1] != PATCH_SIZE or X_test.shape[-2] != PATCH_SIZE:
        raise ValueError(
            f"Expected stored TEST patches of size {PATCH_SIZE}x{PATCH_SIZE}, got {tuple(X_test.shape[-2:])}"
        )
    ...
    # Example optional interpretability export using fold 0 model:
    # alpha_test = predict_scale_weights_on_indices(
    #     X_values=X_test,
    #     X_masks=M_test,
    #     indices=np.arange(len(y_test)),
    #     model_path=model_dir / f"cnn_dre_fold_{fold_ids[0]}.pt",
    # )
    # np.save(model_dir / "test_scale_weights_fold0.npy", alpha_test)
