"""Train and evaluate the single-scale 33x33 CNN-DRE model."""
...
DEFAULT_OUTPUT_DIR = Path("outputs/scale_based_models/scale_33")
...
# =========================================================
# 33x33 ENCODER
# Exact counterpart of the 33x33 branch in the multi-scale model.
# =========================================================
...
class CNNDRE(nn.Module):
    ...
    def __init__(self, in_value_channels, emb_dim=32, hidden_dims=(32,), dropout=0.35, clip_logits=10.0):
        super().__init__()
        self.encoder = PatchEncoder33HybridDil2(in_value_channels, emb_dim)
    ...
    for ep in range(1, MAX_EPOCHS + 1):
        ...
        if auc_ok:
            state = {
                ...
                "model_type": "CNNDRE",
                "encoder_type": "PatchEncoder33HybridDil2",
                "in_value_channels": in_value_channels,
...
def ensemble_predict_on_indices_cnn(
    ...
    for fid in fold_ids:
        ...
        if state.get("encoder_type") != "PatchEncoder33HybridDil2":
            raise RuntimeError(
                f"Encoder mismatch for fold {fid}: expected PatchEncoder33HybridDil2, "
...
# =========================================================
def parse_args():
    parser = argparse.ArgumentParser(description="Train the single-scale 33x33 CNN-DRE model.")
    parser.add_argument(
