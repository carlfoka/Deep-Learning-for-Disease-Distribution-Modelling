# ===== Random Forest baseline for Ecological Niche Modelling (plain RF proba) =====
# Goal: compare CNN-DRE approach vs a traditional ENM RF suitability model.
#
# What this does:
# 1) K-fold CV on point features parquet
# 2) Stage search over n_estimators (TREE_STAGES)
# 3) Per-fold model selection: (optional) AUC floor -> logloss window -> best Boyce (tie-break lower logloss)
# 4) OOF predictions and OOF metrics (Boyce + AUC) using plain RF probabilities
# 5) Ensemble over folds on held-out test using CNN-style softmax(AUC) weights
# 6) Map prediction using *already standardized rasters* (no renormalization), tiled inference
#
# Dependencies:
#   pip install pandas pyarrow scikit-learn numpy rasterio tqdm
#
# NOTE:
# - This is a "traditional RF ENM" baseline: suitability score = P(y=1|x) from RF.
# - No DRE / no prior correction / no log-ratio space ensembling.

import argparse
import os
import warnings
from pathlib import Path
from typing import Sequence, Dict, Tuple, Iterable, Optional
...
except ImportError:  # Allow direct execution: python baselines/random_forest.py
    from baseline_metrics import compute_metrics
...
# Stage grid
TREE_STAGES = (500, 1000, 2000, 3000, 4000)

# RF hyperparams (baseline; tune if desired)
RF_PARAMS = {
    "random_state": 42,
    "n_jobs": -1,
    "bootstrap": True,
...
CLASS_WEIGHT_BALANCED = False  # set True if you want it

# CNN-like selection knobs, adapted to RF
USE_AUC_FLOOR_RF = True
AUC_FLOOR_RF = 0.80

# RF analogue to "loss window": use log-loss (cross-entropy) on validation probs
LOSS_WINDOW_DELTA = 0.03

EPS_PROBA = 1e-7

# CNN-style ensemble weighting (same logic as the MaxEnt implementation)
...
def ensure_mod_label_binary_inplace(df: pd.DataFrame, col=LABEL_COL) -> None:
    if df[col].dtype == object:
        mapper = {
            "presence": 1, "pres": 1, "pos": 1, "positive": 1, "1": 1, "true": 1, "t": 1, "yes": 1,
            "background": 0, "absence": 0, "abs": 0, "bg": 0, "neg": 0, "negative": 0, "0": 0,
            "false": 0, "f": 0, "no": 0
        }
...
def select_numeric_features(
    df: pd.DataFrame,
    label_col: str, fold_col: str, lon_col: str, lat_col: str,
    drop_cols=DROP_COLS
) -> list[str]:
    exclude = {label_col, fold_col, lon_col, lat_col, *drop_cols}
    base = [c for c in df.columns if c not in exclude and np.issubdtype(df[c].dtype, np.number)]
    keep = []
    for c in base:
        v = df[c].to_numpy()
...
def train_rf_fold_stage_search(
    df: pd.DataFrame,
    train_mask: np.ndarray,
    val_mask: np.ndarray,
    feature_cols: Sequence[str],
    tree_stages: Sequence[int],
    rf_params: dict,
    use_auc_floor: bool,
    auc_floor: float,
    ...
    """
    Train RF on train split. Evaluate multiple n_estimators. Select stage:
      - if use_auc_floor: keep stages with AUC >= auc_floor
      - among kept, find min logloss; keep within (min + loss_window_delta)
      - choose max Boyce in window; tie-break lower logloss
      - fallback: best AUC; fallback: first stage
    Returns: (best_model, hist, best_row)
    """
    X_tr = _nan_to_zero(df.loc[train_mask, feature_cols].to_numpy(dtype=float))
    y_tr = df.loc[train_mask, LABEL_COL].to_numpy(dtype=int)

    X_va = _nan_to_zero(df.loc[val_mask, feature_cols].to_numpy(dtype=float))
    y_va = df.loc[val_mask, LABEL_COL].to_numpy(dtype=int)

    hist = {"trees": [], "val_auc": [], "val_boyce": [], "val_logloss": []}

    for t in tree_stages:
        params_t = dict(rf_params)
        params_t["n_estimators"] = int(t)

        rf = RandomForestClassifier(**params_t)
        rf.fit(X_tr, y_tr)

        try:
            p_va = rf.predict_proba(X_va)[:, 1].astype(np.float64)
        except Exception:
            # RF should provide predict_proba, but guard anyway
            p_va = np.full(X_va.shape[0], np.nan, dtype=np.float64)
        ...
        if np.unique(y_va).size > 1 and np.isfinite(p_va).all():
            ...
            ll = float(log_loss(y_va, np.clip(p_va, EPS_PROBA, 1.0 - EPS_PROBA), labels=[0, 1]))
        else:
            auc = np.nan
            boy = np.nan
            ll = np.nan

        hist["trees"].append(int(t))
    ...
    best_trees = int(hist["trees"][best_idx])

    # Refit best model on the same train split
    final_params = dict(rf_params)
    final_params["n_estimators"] = best_trees
    rf_best = RandomForestClassifier(**final_params).fit(X_tr, y_tr)
    ...
    return rf_best, hist, best_row


# ------------------- Cross-validation driver ---------------------------
def run_rf_cv_baseline(
    parquet_path: str,
    fold_col=FOLD_COL,
    label_col=LABEL_COL,
    lon_col=LON_COL,
    ...
    use_auc_floor: bool = USE_AUC_FLOOR_RF,
    auc_floor: float = AUC_FLOOR_RF,
    ...
    feature_cols = select_numeric_features(df, label_col, fold_col, lon_col, lat_col, drop_cols)
    if not feature_cols:
    ...
    models_per_fold: Dict[int, RandomForestClassifier] = {}
    ...
    for k in folds:
        ...
        rf_k, hist, best_row = train_rf_fold_stage_search(
            df=df,
            train_mask=tr_mask,
            val_mask=va_mask,
        ...
        models_per_fold[k_int] = rf_k

        # OOF predictions for this fold's validation rows
        X_va = _nan_to_zero(df.loc[va_mask, feature_cols].to_numpy(dtype=float))
        p_va = rf_k.predict_proba(X_va)[:, 1].astype(np.float64)
        ...
        for t, auc, boy, ll in zip(hist["trees"], hist["val_auc"], hist["val_boyce"], hist["val_logloss"]):
            stage_rows.append({
                "fold": k_int,
                "trees": int(t),
        ...
        print(f"[fold {k_int}] selected trees={best_row['best_trees']} | "
              f"AUC={best_row['best_val_auc']:.4f} Boyce={best_row['best_val_boyce']:.4f} "
              f"logloss={best_row['best_val_logloss']:.4f}")

    # OOF metrics (report this as CV metric)
    ...
    stage_df = pd.DataFrame(stage_rows).sort_values(["fold", "trees"]).reset_index(drop=True)

    print("\n===== RF BASELINE CV OOF METRICS =====")
    print(oof_metrics)
    print("\nPer-fold summary:")
    print(cv_df)

    # Persist
    cv_df.to_csv("rf_baseline_cv_summary.csv", index=False)
    stage_df.to_csv("rf_baseline_cv_stage_history.csv", index=False)
    pd.DataFrame([oof_metrics]).to_csv("rf_baseline_cv_oof_metrics.csv", index=False)
    np.save("rf_baseline_oof_pred.npy", oof_pred)
...
def softmax_auc_weights_from_cv_auc(
    ...
    # Same logic as the MaxEnt/CNN implementation: center by mean AUC,
    # multiply by temperature, and apply a numerically stable softmax.
...
def evaluate_rf_on_test_ensemble(
    models_per_fold: Dict[int, RandomForestClassifier],
    feature_cols: Sequence[str],
    test_path: str,
    ...
    X_te = _nan_to_zero(
        df_te.reindex(columns=list(feature_cols), fill_value=0.0).to_numpy(dtype=float)
    )
    ...
    for i, mdl in enumerate(models):
        P[i, :] = mdl.predict_proba(X_te)[:, 1].astype(np.float64)
    ...
    print("\n===== HELD-OUT TEST METRICS (RF SOFTMAX-AUC ENSEMBLE) =====")
...
# ------------------- Mapping: plain RF ensemble suitability ---------------------------
def build_rf_suitability_map_ensemble_prestandardized(
    raster_dir: str,
    landmask_tif: str,
    out_dir: str,
    models_per_fold: Dict[int, RandomForestClassifier],
    ...
    output_name_mean: str = "rf_baseline_suitability_mean_softmax_auc.tif",
):
    """
    Assumes rasters in raster_dir are ALREADY standardized (e.g., median/IQR)
    exactly as used during point feature extraction.
    Suitability score = softmax(AUC)-weighted mean of RF probabilities
    across fold models.
    """
    ...
    for r0, r1, c0, c1 in tqdm(tiles(H, W, tile), total=n_tiles, desc="RF baseline map"):
        lm_tile = landmask[r0:r1, c0:c1]
        if landmask_value is not None and not np.any(lm_tile == landmask_value):
            continue

        rc = np.argwhere(lm_tile == landmask_value) if landmask_value is not None else np.argwhere(np.isfinite(lm_tile))
        if rc.size == 0:
            continue
        rr, cc = rc[:, 0], rc[:, 1]
        n = rr.size
        ...
        # per-model probabilities and weighted mean
        p_ens = np.zeros(n, dtype=np.float64)
        for k, mdl in enumerate(models):
            p = mdl.predict_proba(X)[:, 1].astype(np.float64)
            p_ens += w[k] * p
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate the Random Forest ENM baseline."
    )
    parser.add_argument(
        "--train-path",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/baselines/random_forest"),
...
def main() -> None:
    ...
    cv_out = run_rf_cv_baseline(
        ...
        rf_params=RF_PARAMS,
        class_weight_balanced=CLASS_WEIGHT_BALANCED,
        use_auc_floor=USE_AUC_FLOOR_RF,
        auc_floor=AUC_FLOOR_RF,
        loss_window_delta=LOSS_WINDOW_DELTA,
    )
    ...
    test_metrics, test_scores, test_labels, used_features = evaluate_rf_on_test_ensemble(
        models_per_fold=models_per_fold,
    ...
    # 4) Optional mapping (rasters must already be standardized)
    if args.raster_dir is not None:
        map_dir = args.output_dir / "suitability_maps"
        build_rf_suitability_map_ensemble_prestandardized(
            ...
            output_name_mean="rf_suitability_mean_softmax_auc.tif",
        )
